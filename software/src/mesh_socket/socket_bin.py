...
MAX_BYTES = 219 # TODO it should be 220
MAX_CHUNKS = 999
MAX_RETRIES = 10
...
class MeshPayload:
    def __init__(self, connection_id: str, flag: set[SocketFlag], data: Optional[bytes] = None, meta: dict[str, Any] = None):
        self.flag = flag
        self.data = data or b""
        self.connection_id = connection_id  # e.g. "!bd4f9c68"
        self.meta = meta or {}

    def encode(self) -> bytes:
        flags = sum(f.value for f in self.flag)
        meta_flags = 0
        fields = []

        if self.meta.get("compression"):
            meta_flags |= 0b00000001
            fields.append(struct.pack("B", 1 if self.meta["compression"] == "zlib" else 0))

        if "chunk_index" in self.meta:
            meta_flags |= 0b00000010
            fields.append(struct.pack(">H", self.meta["chunk_index"]))

        if "total_chunks" in self.meta:
            meta_flags |= 0b00000100
            fields.append(struct.pack(">H", self.meta["total_chunks"]))

        conn_bytes = bytes.fromhex(self.connection_id.lstrip("!"))

        header = struct.pack(">HB4s", flags, meta_flags, conn_bytes)
        return header + b"".join(fields) + self.data

    @classmethod
    def decode(cls, packet: bytes):
        flags, meta_flags, conn_bytes = struct.unpack(">HB4s", packet[:7])
        connection_id = "!" + conn_bytes.hex()
        offset = 7
        meta = {}

        if meta_flags & 0b00000001:
            comp, = struct.unpack("B", packet[offset:offset + 1])
            meta["compression"] = "zlib" if comp == 1 else None
            offset += 1
        if meta_flags & 0b00000010:
            meta["chunk_index"], = struct.unpack(">H", packet[offset:offset + 2])
            offset += 2
        if meta_flags & 0b00000100:
            meta["total_chunks"], = struct.unpack(">H", packet[offset:offset + 2])
            offset += 2

        data = packet[offset:]

        if meta.get("compression") == "zlib" and "chunk_index" not in meta:
            data = zlib.decompress(data)
        flags_set = {f for f in SocketFlag if f.value & flags}
        return cls(connection_id=connection_id, flag=flags_set, data=data, meta=meta)
...
class MeshSocket:
    ...
    def _write(self, payload: MeshPayload, compress: bool = False):
        ...
        dummy_json_bytes = dummy_payload.encode()
        ...
        for i in range(total_chunks):
            ...
            while retries < MAX_RETRIES:
                # Send the chunk
                self._send(chunk_payload.encode(), f"chunk {i + 1}/{total_chunks}")
                ...
                def _on_ack(packet, interface=None):
                    try:
                        ...
                        ack_payload = MeshPayload.decode(ack_str)
    ...
    def _send_ack(self, chunk_index: int):
        ...
        print(f"[DEBUG] Sending ACK payload: {ack_payload}")
        self._send(ack_payload.encode(), f"ACK {chunk_index}")
    ...
    @staticmethod
    def connect(iface, remote_node_id: str, timeout: int = TIMEOUT_SECONDS) -> Optional["MeshSocket"]:
        ...
        def on_receive(packet, interface=None):
            ...
            try:
                ...
                payload = MeshPayload.decode(msg)
    ...
    def _on_receive(self, packet, interface=None):
        ...
        if SocketFlag.CONN_REQUEST in payload.flag:
            ...
            server_id = self.iface.getMyNodeInfo()["user"]["id"]
